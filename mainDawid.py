...
def wczytaj_z_pliku(okno):  #funkcja wybierająca plik
    ...
    przyciskPrzegladaj.grid(row=2, column=6)
    przyciskSzukaj.grid(row=1, column=6)
...
def otwieranie_pliku(okno, file_path): #funkcja wybierająca konkretny ciag z pliku
    for widget in okno.winfo_children():
        widget.destroy()
    ramka1 = CTkScrollableFrame(okno, width=(size[0] / 15) * 12, height=(size[1] - 50))
    Bwidth = (size[0] / 15) * 12  # zmienna przechowująca szerokość naszej ramki
    ramka1.grid(row=1, column=0, columnspan=4, sticky=E + W + N + S)
    ...
    if file_path.endswith(".txt"):
        with open(file_path, 'r') as plik:
            zawartosc = plik.read()
        if zawartosc.startswith(">"): #jeśli plik tekstowy zaczyna się od >
            headers = []
            sequences = []
            ...
            for string in headers:
                x = CTkButton(ramka1, text=headers[i], command=lambda numer=i: szukaj_w_pliku(okno, file_path, sequences[numer]))
                x.grid(row=i, column=0, sticky=W+E)
                i+=1
        else:
            szukaj_w_pliku(okno, file_path, zawartosc)
    elif file_path.endswith(".fasta"): #plik fasta
        i=0
        with open(file_path, "r") as handle:
            for record in SeqIO.parse(handle, "fasta"):
                x = CTkButton(ramka1, text=record.id,command=lambda sekwencja=record.seq: szukaj_w_pliku(okno, file_path, sekwencja))
                x.grid(row=i, column=0, sticky=W + E)
                i+=1

def szukaj_w_pliku (okno, file_path, sequence):#funkcja wypisujaca wszystkie lancuchy w interfejsie
    for widget in okno.winfo_children():
        widget.destroy()
