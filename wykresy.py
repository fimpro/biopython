...
def rysowanie_wykresu(okno, lanc_Kodonow): #funkcja rysująca wykres ze zmiennymi danymi
    ...
    dane_dostępność = {'Aminokwasy': aminokwas, 'Dostępność powierzchniowa': wartości_dostępność} #zamiana danych na przystępne do załadowania do wykresu
    ...
    Dane_indeks_hydrofobowy = Dane_indeks_hydrofobowy[['Aminokwasy', 'Indeks hydrofobowy']].groupby('Aminokwasy').sum() #pogrupowanie danych
    if (c1):
        Dane_indeks_hydrofobowy.plot(kind='line', legend=False, ax=Wypisz_wykres_cech, color='#581266') #narysowanie wykresu od danych, jeśli odpowiadający checkbox jest zaznaczony
    ...
    Wypisz_wykres_cech.yaxis.set_minor_locator(MultipleLocator(1)) #ustawienie pomniejszych 'ticków' na osi y w odległości 1 od siebie
    ...
    Dane_aminokwasów = Dane_aminokwasów[['Aminokwas', 'Częstotliwość występowania']].groupby('Aminokwas').sum() #grupowanie danych
...
def wykresy(ramex, wzor, lanc_Kodonow):  # robocza funkcja do wykresów
    ...
    DaneButton = CTkButton(master=ramka1, text="Dane", command=lambda: dane_interfejs(okno, wzor, lanc_Kodonow), width=100)
...
def dane_interfejs(ramex, wzor, lanc_Kodonow):
    ...
    linijka_4 = CTkLabel(okno, text=str("- "+str(round(struktura[2] * 100,3)) + " % beta zakrętów (pętli omega)")) #wypisanie wszystkich potrzebnych danych
